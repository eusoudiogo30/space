...
adminRouter.get('/fraud-alerts/:id', asyncHandler(async (req, res) => res.json({ alert: await prisma.fraudAlert.findUniqueOrThrow({ where: { id: String(req.params.id) }, include: { game: true, user: true } }) })))
